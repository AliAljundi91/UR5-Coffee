...
import time
from positions1 import POSITIONS
...
def moveCrude():
    moveJ(POSITIONS["boxCrudeJ"]) # Move to crude box position
...
def packBag(bagType, bagNr):
    ...
    moveCrude()
    moveJ(POSITIONS[bagType]["bags"][bagKey]["placeFine"], MOVING_SPEED, MOVING_ACCEL)
    ...
    moveJ(POSITIONS[bagType]["bags"][bagKey]["placeFine"])
    moveCrude()

def pushNeeded(bagType, bagNr):
    if bagType == "1000g" and bagNr in [2, 3, 6, 7]:
        return True
    else:
        return False
...
def pushBag(bagType, bagNr):
    bagKey = f"bag{bagNr}"
    moveJ(POSITIONS[bagType]["push3"][bagKey]["quarter"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["half"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["full"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["down"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["orient"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["push"], MOVING_SPEED, MOVING_ACCEL)
    moveJ(POSITIONS[bagType]["push3"][bagKey]["down"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["push"], MOVING_SPEED, MOVING_ACCEL)
    moveJ(POSITIONS[bagType]["push3"][bagKey]["down"])
    moveJ(POSITIONS[bagType]["push3"][bagKey]["full"])
