...
import time
from positions4 import POSITIONS
...
SSL_ACCEL = 0.35*NORMAL_ACCEL_L
SSJ_ACCEL = 0.40*NORMAL_ACCEL_J

LSL_ACCEL = 0.60*NORMAL_ACCEL_L
LSJ_ACCEL = 1.00*NORMAL_ACCEL_J

UDL_ACCEL = 1.00*NORMAL_ACCEL_L
UDJ_ACCEL = 1.00*NORMAL_ACCEL_J
DROP_ACCEL = 0.20*NORMAL_ACCEL_L

PU_ACCEL = 0.40*NORMAL_ACCEL_L

PUSH_ACCEL = 0.20*NORMAL_ACCEL_L

payload = 2.29  # Initial payload of the gripper

def moveJ(coordinate, accel=NORMAL_ACCEL_J):
    success = rtde_c.moveJ(coordinate, speed=NORMAL_SPEED_J, acceleration=accel)
    #print("Move executed:", coordinate)

def moveL(coordinate, accel=NORMAL_ACCEL_L):
    success = rtde_c.moveL(coordinate, speed=NORMAL_SPEED_L, acceleration=accel)
    #print("Move executed:", coordinate)

def wait(seconds):
    time.sleep(seconds)

def moveHome(accel=NORMAL_ACCEL_J):
    rtde_c.setPayload(payload, [0.0, 0.0, 0.0])
    moveJ(POSITIONS["homeJ"], accel) # Move to home position
    print("Robot initialized and moved to home position.")

def moveCrude(accel=NORMAL_ACCEL_L):
    moveL(POSITIONS["boxCrude"], accel) # Move to crude box position
...
def moveToPickUp(bagType, bagNr):
    if bagType == "1000g":
        ...
        moveL(POSITIONS[bagType]["pickup"], PU_ACCEL) # Move to 1000g bag position
        wait(0.2) # Short wait to ensure the bag is gripped
        moveL(POSITIONS["home"], UDL_ACCEL) # Move back to angled home position
...
def packBag(bagType, bagNr):
    ...
    moveCrude(LSL_ACCEL)
    moveJ(POSITIONS[bagType]["bags"][bagKey]["placeFineJ"], SSJ_ACCEL)
    moveL(POSITIONS[bagType]["bags"][bagKey]["dropOff"], DROP_ACCEL)
...
def pushBag(bagType, bagNr):
    ...
    moveL(POSITIONS[bagType]["push3"][bagKey]["pushb"], PUSH_ACCEL)
    moveL(POSITIONS[bagType]["push3"][bagKey]["pushf"], PUSH_ACCEL)
    moveL(POSITIONS[bagType]["push3"][bagKey]["pushb"], PUSH_ACCEL)
    moveL(POSITIONS[bagType]["push3"][bagKey]["full"])
    moveL(POSITIONS[bagType]["push3"][bagKey]["half"])
    moveL(POSITIONS[bagType]["push3"][bagKey]["quarter"])
